# Grand Edge Developer Guide

This page keeps the developer setup, command, architecture, and planning notes
that previously lived in the front README.
...
- [No-Docker local setup](running/no-docker.md)
- [Docker setup](running/docker.md)
- [ML workflow](running/ml-workflow.md)
...
- `crates/ingest`: the OSRS Wiki ingestion boundary.
...
- Deterministic fixtures live under `tests/fixtures/` and crate-local
  `tests/fixtures/`.
- Network-free OSRS Wiki client coverage should use `wiremock`, not live API
  calls.
...
The shared runtime configuration layers `configs/default.toml`, optional
...
contact address for OSRS Wiki API access.
...
The repository includes a research-only Python workspace under `ml/`. Python may
load Rust-produced dataset exports, train offline experiments, and export
artifact bundles for Rust validation, but it must not serve live recommendations
or become a runtime dependency of the Rust production path.
...
artifact validation/runtime. See [ML workflow](running/ml-workflow.md).
